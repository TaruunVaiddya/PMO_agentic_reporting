...
export function AppSidebar({
  ...
  const handleSaveRename = async (sessionId: string) => {
    ...
    setIsRenaming(true)
    try {
      const response = await fetchWithAuth(`${process.env.NEXT_PUBLIC_API_URL}/rename_session`, {
        method: 'PATCH',
        headers: {
  ...
  const handleToggleFavorite = async (sessionId: string, currentIsFavorite: boolean) => {
    ...
    try {
      const response = await fetchWithAuth(`${process.env.NEXT_PUBLIC_API_URL}/toggle_favorite`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const confirmDeleteSession = async () => {
    ...
    try {
      const response = await fetchWithAuth(`${process.env.NEXT_PUBLIC_API_URL}/sessions/${sessionToDelete.id}`, {
        method: 'DELETE',
      })
